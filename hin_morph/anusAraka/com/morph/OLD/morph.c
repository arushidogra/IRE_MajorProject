...
void fun_morph(morph,swit1,fpt,fp_pdgm,DBM_FLAG,db_lex,db_suff,ans_ar)
FILE *fpt,*fp_pdgm;
#ifdef GNU
...
for(l= morph_size;l>=0;l--)
	...
	/* This condition is added to avoid infinite loop. In case of blank suffix with 'ixi' addition , the test_hypothetical_dict was going in infinite loop. */
	if(!strcmp(suff,""))
