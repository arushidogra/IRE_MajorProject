...
#include "/home/arushi/Desktop/MajorProject/temp_pr/IRE_MajorProject/hin_morph/anusAraka/com/morph/headers/glbl.h"
...
#ifdef GNU
GDBM_FILE db_dict1,db_dict2,db_suff,db_uword;
#else
DBM *db_dict1,*db_dict2,*db_suff,*db_uword;
#endif

char morph[Morphsize];
char fpath[300],fname[300],fpath_dict1[300],fpath_dict2[300];
...
if (argc < 3 )
{
printf("Usage morph.out <pdgm_offset_info> <dict_path> <dict_path> <uword_dict_path> [UFPADL]\n");
exit(0);
}
if(argc > 5)
{
...
strcpy(fpath,argv[1]);
strcpy(fpath_dict1,argv[2]);
strcpy(fpath_dict2,argv[3]);
strcpy(upath,argv[4]);
...
fgetword(fp_prop_noun_dict,line,',');
prop_noun_lex_ar[i].priority = 49; /* 48+1; char -> int */
i++;
}
...
if(DBM_FLAG)
{
strcpy(fname,fpath_dict1);
#ifdef GNU
db_dict1 = gdbm_open(fname,512,GDBM_READER,0666,my_fatal());
#else
db_dict1 = dbm_open(fname,O_RDONLY,0666);
#endif

if(DBM_FLAG)
{
strcpy(fname,fpath_dict2);
#ifdef GNU
db_dict2 = gdbm_open(fname,512,GDBM_READER,0666,my_fatal());
#else
db_dict2 = dbm_open(fname,O_RDONLY,0666);
#endif
}
 ...
 while (1)
  {
        if(c == EOF)
	  {
          fun_close(fpt1,fp_pdgm,swit1,DBM_FLAG,db_dict1,db_dict2,db_suff);
          exit(0);
	  }
        else 
           {
		...
		printf("%s",morph);/* print the marked words as they are */
		    ...
		    {
        	     fun_morph(morph,swit1,fpt1,fp_pdgm,DBM_FLAG,db_dict1,db_suff,ans_ar);
                     if(ans_ar[0].offset == -2)
        	     fun_morph(morph,swit1,fpt1,fp_pdgm,DBM_FLAG,db_dict2,db_suff,ans_ar);
                     if(ans_ar[0].offset == -2) 
                       {
