...
char fpath[80],fname[80],fpath_dict[80];
char upath[80],uname[80];
char line[100];
...
if (argc < 3 )
{
printf("Usage morph.out <pdgm_offset_info> <dict_path> <uword_dict_path> [UFPADL]\n");
exit(0);
}
if(argc > 4)
{
          ...
          exit(0);
        ...
        else 
           {
            ...
            if ((strcmp(morph,"\0")))
         	{
                FINDONCE = 0;
                if((LINE_NUM_FLAG && snt_num(morph)) && (!HORI_OUTPUT)) print_snt_num(morph);
		else 
                {
                WORD_NUM++;
                if(FOR_USER)
