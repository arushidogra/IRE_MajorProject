...
extern struct suff_info aux_suff_info_ar[];
extern int sizeof_aux_suff;
extern void comp_verb_heuristics();
extern void get_derived_rt();
extern int DEBUG;

int 
comp_verbs(root,pdgm,DBM_FLAG,db_suff,db_lex,ans_root,aux_verb)
...
ans_root[0] = '\0';
new_root[0] = '\0';
comp_verb_heuristics(root,pdgm,tmp_aux_verbs,new_root,pre_kqxaMwa);
if(strcmp(tmp_aux_verbs,""))
{
get_derived_rt(new_root,aux_suff_info_ar->suffix,sizeof_aux_suff,sizeof(aux_suff_info_ar[0]),strcmp,DBM_FLAG,db_suff,db_lex,"verb",ans_root,aux_verbs,cur_aux_str);
if(strcmp(ans_root,"")) 
{
if(!strcmp(cur_aux_str,pre_kqxaMwa))
if(strcmp(aux_verb,""))
{
strcat(aux_verbs,"_");
