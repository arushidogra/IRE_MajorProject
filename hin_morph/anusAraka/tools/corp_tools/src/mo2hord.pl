...
sub mo2hor {
  ...
  while (<$infile>) {
    ...
    for ($col = 1;  $col <= $#values; $col += (3 + $numflds{$values[$col]}) ) {
      $categ = $values[$col];
      if ($categ eq 'AVY') { print '                              '; }
      if (($categ eq 'v')|($categ eq 'v_a')|($categ eq 'vn')) {
        $position = $fldpos{$categ,tam};
        print $values[$col + 1], '_', $values[$col + 2 + $position], '{';   
                 # print root and tam and '{'
      }
      elsif ($categ eq 'n') {
        $position = $fldpos{$categ,parsarg};
        print $values[$col + 1], '_', $values[$col + 2 + $position], '{';   
                 # print root and parsarg and '{'
      }
      else { print $values[$col + 1], '{'; }   # print root and '{'
      print $categ;          # print lex cat name
      for ($i=0;  $i <= $numflds{$categ};  $i++) {
