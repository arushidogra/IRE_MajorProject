WRD [A-Za-z][A-Za-z]*
%%
OUT_¤[^¤]*_OUT { printf("%s",yytext);}
FRZ_¤[^¤]*_FRZ { printf("%s",yytext);}
FRZ_MO_¤[^¤]*_MO_FRZ { printf("%s",yytext);}
#_LWG_¤[^#]*_LWG_# { printf("%s",yytext);}
#LWG_¤[^#]*_LWG# { yytext[yyleng-5] = '\0'; srch_dbm1(yytext+6);}
#LWG_[^#]*_LWG# { printf("%s",yytext);}
#[^#]*# { printf("%s",yytext);}
@[^ \n]*/[ 	\n] { printf("%s",yytext);}
{WRD}	{srch_dbm(yytext);}
%%
