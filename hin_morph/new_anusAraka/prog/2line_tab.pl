...
$out_len = length($hindi_wrd_lst[$i]);
	if($in_len < $out_len){
	  ...
	  $hindi_wrd_lst[$i] = $hindi_wrd_lst[$i]."\t";
	}
	if($out_len < $in_len){
 	  $hindi_wrd_lst[$i] = &add_spaces($hindi_wrd_lst[$i],$in_len-$out_len);
	  $telugu_wrd_lst[$i] = $telugu_wrd_lst[$i]."\t";
	}
	if(($out_len == $in_len) && ($telugu_wrd_lst[$i] ne "__NEW-LINE__")){
	$telugu_wrd_lst[$i] = $telugu_wrd_lst[$i]."\t";
	$hindi_wrd_lst[$i] = $hindi_wrd_lst[$i]."\t";
	}
}
...
{
if($telugu_wrd_lst[$i] =~ /[^\t]\t$/){
$char_count = $char_count+((int(length($telugu_wrd_lst[$i])/8)+1)*8);
}
else{
$char_count = $char_count+((int(length($hindi_wrd_lst[$i])/8)+1)*8);
}
if(($char_count > 80) || ($preceding_white_space[$i] eq "\n")){ 
print OUT1 "\n";
print OUT2 "\n";
if($telugu_wrd_lst[$i] =~ /[^\t]\t$/){
$char_count = (int(length($telugu_wrd_lst[$i])/8)+1)*8;
}
else{
$char_count = (int(length($hindi_wrd_lst[$i])/8)+1)*8;
}
}
print OUT1 $telugu_wrd_lst[$i];
print OUT2 $hindi_wrd_lst[$i];
$i++;
}
print OUT1 "\n";
print OUT2 "\n";
...
local($i,$len,$tab1,$tab2,$no_of_tabs);

$len = length($strng);
$tab1 = int(($len+$number_of_blanks)/8);
$tab2 = int($len/8);
$no_of_tabs = $tab1-$tab2+1;
